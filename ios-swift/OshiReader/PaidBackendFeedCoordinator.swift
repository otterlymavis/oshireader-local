...
final class PaidBackendFeedCoordinator: ObservableObject {
    ...
    func refresh(
        ...
        do {
            ...
            // An empty synchronized term list performs no hosted fetch. Do not
            // establish a cursor in that state: otherwise adding the profile's
            // first hosted term later would incorrectly request only the
            // incremental overlap and skip its silent 180-day baseline.
            if !backendTermIDs.isEmpty {
